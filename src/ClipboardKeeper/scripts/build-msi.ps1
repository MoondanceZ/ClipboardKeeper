...
$project = Join-Path $root "ClipboardKeeper.csproj"
...
function Get-ProjectVersion {
    ...
    if ([string]::IsNullOrWhiteSpace($version)) {
        throw "ClipboardKeeper.csproj does not contain a <Version> value."
    }
...
if (-not $SkipPublish) {
    Write-Host "Publishing ClipboardKeeper AOT ($Configuration, $Runtime)..."
...
$msi = Join-Path $artifacts "ClipboardKeeper-$version-x64.msi"
